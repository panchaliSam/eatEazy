...
const createMenuItem = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "Menu item created successfully", menuItemId: newMenuItem });
    } catch (error) {
...
const getMenuItemsByRestaurantId = async (req, res) => {
    try {
        const menuItems = await MenuService.getMenuItemsByRestaurantId(req.params.restaurantId);
...
const updateMenuItemById = async (req, res) => {
    try {
        await MenuService.updateMenuItemById(req.params.menuItemId, req.user.id, req.body);
