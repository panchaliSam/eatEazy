...
const createMenuItem = async (menuItemData) => {
  ...
  const restaurantExists = await RestaurantModel.findByRestaurantId(
    restaurantId
  );
  if (!restaurantExists) {
  ...
  const existingMenuItem = await MenuModel.getMenuItemsByRestaurantId(
    restaurantId
  );
  const menuItemExists = existingMenuItem.some(
...
const updateMenuItemById = async (menuItemId, userId, menuItemData) => {
  const menuItem = await MenuModel.updateMenuItemById(menuItemId);

  if (!menuItem) {
    throw new Error("Menu item not found");
  }

  const restaurant = await RestaurantModel.findByRestaurantId(
